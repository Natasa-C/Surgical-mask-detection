...
class Problem():
    ...
    TRAIN_MFCC_PATH = './clean_data/train_mean_mfcc.csv'
    VALIDATION_MFCC_PATH = './clean_data/validation_mean_mfcc.csv'
    TEST_MFCC_PATH = './clean_data/test_mean_mfcc.csv'
    ...
    def loadMeanAndFeaturesForDataSet(self, filename_features,filename_mean, data_names):
        ...
        for line in tqdm(fd.readlines()[1:]):
            ...
            if name in data_names:
                numeric_data_features = [float(elem) for elem in features[1:]]
                data_features[data_names.index(name)].extend(numeric_data_features)
    ...
    def svcAlgorithm(self):
        ...
        model = SVC(probability=True, C=10, gamma='scale', kernel='rbf',
        ...
        skplt.metrics.plot_confusion_matrix(self.validation_labels, predictions)
        plt.show()
