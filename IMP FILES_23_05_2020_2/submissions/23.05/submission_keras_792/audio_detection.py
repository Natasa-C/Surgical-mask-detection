...
class Problem():
    ...
    OUTPUT_FILE_PATH = './submission_output.txt'
    ...
    def loadMeanAndFeaturesForDataSet(self, filename_features, filename_mean, data_names):
        ...
        for line in tqdm(fd.readlines()[1:]):
            ...
            if name in data_names:
                numeric_data_features = [float(elem) for elem in features[1:]]
                # numeric_data_features.append(float(features[6]))
                # numeric_data_features.append(float(features[10]))
                # numeric_data_features.append(float(features[12]))
                data_features[data_names.index(name)] = numeric_data_features
    ...
    def svcAlgorithm(self):
        ...
        print(self.TRAIN_FEATURES_PATH)
    ...
    def kerasNeuralNetwork(self):
        ...
        model = Sequential()
        model.add(Dense(units=110, input_dim=55, activation='relu')) # input layer requires input_dim param
        model.add(Dropout(.2))
        ...
        model.add(Dropout(.2))
        model.add(Dense(units=1, activation='sigmoid')) # sigmoid instead of relu for final probability between 0 and 1
        ...
        model.fit(self.train_features, self.train_labels, epochs = 200, batch_size=50)

        # The evaluate method calculates the losses and metrics for the trained model
        # Evaluating the model on the training set
        score = model.evaluate(self.train_features, self.train_labels, verbose=0)
        print("Training Accuracy: {0:.3%}".format(score[1]))

        # Evaluating the model on the validation set
        score = model.evaluate(self.validation_features, self.validation_labels, verbose=0)
        print("Validation Accuracy: {0:.3%}".format(score[1]))

        # Predict labels for validation data set
        predictions = model.predict_classes(self.validation_features, verbose=0)
